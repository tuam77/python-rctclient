...
def _decode_timeseries(data: bytes) -> Tuple[datetime, Dict[datetime, int]]:
    ...
    '''
    timestamp = datetime.fromtimestamp(struct.unpack('>I', data[0:4])[0], timezone.utc)
    ...
    for pair in range(0, int(len(data) / 4 - 1), 2):
        pair_ts = datetime.fromtimestamp(struct.unpack('>I', data[4 + pair * 4:4 + pair * 4 + 4])[0], timezone.utc)
...
def _decode_event_table(data: bytes) -> Tuple[datetime, Dict[datetime, EventEntry]]:
    ...
    for pair in range(0, int(len(data) / 4 - 1), 5):
        ...
        timestamp = datetime.fromtimestamp(struct.unpack('>I', data[4 + pair * 4 + 4:4 + pair * 4 + 8])[0], timezone.utc)
